...
public class BookStore {
    ...
    static {
        books.add(new NovelBook("天龙八部", 5250, "金庸"));
        books.add(new NovelBook("巴黎圣母院", 5666, "雨果"));
        books.add(new NovelBook("悲惨世界", 3600, "雨果"));
        books.add(new NovelBook("金瓶梅", 4600, "兰陵笑笑生"));
    }
    ...
    public static void main(String[] args) {
        ...
        for (IBook iBook : books) {
            System.out.println("书籍名称：《" + iBook.getName() + "》, 书籍作者：" + iBook.getAuthor() +
                    ", 书籍价格" + numberFormat.format(iBook.getPrice() / 100.0) + "元");
        }
